...
# ─────────────────────────────────────────────
MAX_DRAWDOWN_LIMIT = 0.035  # scaling reference point (NOT a hard stop — trades always continue)
...
def dynamic_position(confidence: float, equity: float, peak: float) -> float:
    # NO hard stop — always trade, just scale size by drawdown
    current_dd = (peak - equity) / peak if peak > 0 else 0.0
    dd_ratio   = min(current_dd / MAX_DRAWDOWN_LIMIT, 1.0)  # cap at 1.0, never stop
    ...
    return max(equity * 0.01, min(equity * kelly, equity * max_risk))  # always at least 1%
...
def run_backtest(df: pd.DataFrame):
    ...
    for i in range(TRAIN_WINDOW, len(df) - HOLD_DAYS):
        ...
        # No hard stop — just scale position based on drawdown, always trade
